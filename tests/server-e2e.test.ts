...
describe("Android server E2E source driver", () => {
  ...
  it("dispatches real page, history, search, read and message lifecycle calls from LaunchActivity", async () => {
    ...
    expect(patched).toContain('"reaction-actors".equals(command)');
    expect(patched).toContain("TL_messages_getMessagesReactions");
    expect(patched).toContain("TL_messages_getMessageReactionsList");
    expect(patched).toContain("reaction_actors_ready total=");
    expect(patched).toContain('getDeclaredField("users")');
    expect(patched).toContain('"reaction-panel".equals(command)');
    expect(patched).toContain("new org.telegram.ui.Components.ReactionsContainerLayout(");
    expect(patched).toContain("panel.setDelegate(new org.telegram.ui.Components.ReactionsContainerLayout.ReactionsContainerDelegate()");
    expect(patched.indexOf("panel.setLayoutParams(params)")).toBeLessThan(
      patched.indexOf("panel.setMessage(target, chat.getCurrentChatInfo(), false)"),
    );
    expect(patched).toContain("reaction_panel_opened visible=");
    expect(patched).toContain("holder.enterImageView.getImageReceiver()");
    expect(patched).toContain("holder.loopImageView.animatedEmojiDrawable.getImageReceiver()");
    expect(patched).toContain("reaction.static_icon, reaction.appear_animation, reaction.select_animation");
    expect(patched).toContain("documentsToClear.putAll(coldDocuments)");
    expect(patched).toContain("reaction_panel_compact_loaded holders=");
    expect(patched).toContain('getDeclaredMethod("showCustomEmojiReactionDialog")');
    expect(patched).toContain("dialog.emojiGridView");
    expect(patched).toContain("reaction_panel_expanded_progress cells=");
    expect(patched).toContain("getPathToAttach(networkDocument, true).exists()");
    expect(patched).toContain("loadedPositions.size() == itemCount");
    expect(patched).toContain("expanded_files=");
    expect(patched).toContain("expanded_cells=");
    expect(patched).toContain("reaction_panel_failed reason=timeout");
  ...
  it("sends follow-up commands straight to the running LaunchActivity", async () => {
    ...
    expect(runner).toContain('if (command === "sticker-uninstall")');
    expect(runner).toContain('"messages.uninstallStickerSet"');
    ...
    expect(runner).toContain('"chat", "send", "search", "read", "draft", "reply", "edit", "delete", "forward", "reaction", "reaction-actors", "reaction-panel", "download"');
    ...
    expect(runner).toContain('dispatch(launchComponent, e2eAction, "reaction-actors"');
    expect(runner).toContain('waitForOutcome("reaction_actors_ready", "reaction_actors_failed"');
    expect(runner).toContain('const minimumActors = Number(option("min-actors", "1"))');
    expect(runner).toContain("total < minimumActors");
    expect(runner).toContain("Android reaction actor preview incomplete");
    expect(runner).toContain('dispatch(launchComponent, e2eAction, "reaction-panel"');
    expect(runner).toContain('const coldReactionPanel = command === "reaction-panel"');
    expect(runner).toContain('const reactionPanelLoadTimeout = coldReactionPanel ? 300_000 : 90_000');
    expect(runner).toContain('if (coldReactionPanel) {');
    expect(runner).toContain('adb(["shell", "am", "force-stop", packageName]);');
    expect(runner).toContain('waitForOutcome("reaction_panel_opened", "reaction_panel_failed"');
    expect(runner).toContain('"reaction_panel_loaded",');
    expect(runner).toContain('reactionPanelLoadTimeout,');
    expect(runner).toContain("Android reaction panel incomplete");
    expect(runner).toContain("const expandedCells = Number(loaded.expanded_cells)");
    expect(runner).toContain("expandedCells !== expandedItems");
    expect(runner).toContain("expandedFiles <= 0");
