#!/bin/bash
# Derived from NextAlone/Nagram@4aadabff30a6252705c6c5f03658fb5f0c4af4ae.
# Kept here so native x86 dependency builds are reproducible.
set -e
...
function build {
	for arg in "$@"; do
        ...
        ANDROID_API=21
				...
				BIN_MIDDLE=android
				CPU=x86_64
				OPTIMIZE_CFLAGS="-O3" # CROSSGRAM NDK-compatible x86_64 flags
        ...
        ANDROID_API=21
				...
				BIN_MIDDLE=android
				CPU=i686
				OPTIMIZE_CFLAGS="-O3" # CROSSGRAM NDK-compatible x86 flags
...
if (( $# == 0 )); then
	build ${CROSSGRAM_NATIVE_TARGETS:-arm64 arm}
else
	build $@
fi
