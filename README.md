...
上传本地文件时，修改版客户端会在后台一次计算 MD5、SHA-1、每 1 MiB 的 cumulative
SHA-1 checkpoint 和前 10 MiB MD5，再调用 `crossgram.prepareMediaUploadV2` 查询 QQ
是否命中秒传并允许服务端并行流式上传 Highway。命中时继续复用原有
